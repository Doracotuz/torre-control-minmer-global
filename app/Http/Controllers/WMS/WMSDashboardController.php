...
class WMSDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $skusWithStock = (clone $basePalletItemQuery)->distinct('product_id')->count();

        $totalStorageLocations = (clone $baseLocationQuery)->where('type', 'storage')->count();
        
        if ($areaId) {
            $occupiedStorageLocations = Location::where('type', 'storage')
                ->whereHas('pallets.purchaseOrder', function($q) use ($areaId) {
                    $q->where('area_id', $areaId);
                })
