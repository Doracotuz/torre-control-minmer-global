                        ...
                        @forelse ($areas as $area)
                            <div class="bg-white rounded-lg shadow border border-gray-100 p-5 flex flex-col transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                                <div class="flex justify-between items-start mb-3">
                                    ...
                                    @if ($area->icon_path)
                                        <img class="h-10 w-10 object-contain" src="{{ Storage::disk('s3')->url($area->icon_path) }}" alt="{{ $area->name }}">
                                    @else
                                ...
                                @forelse ($areas as $area)
                                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if ($area->icon_path)
                                                <img class="h-9 w-9 object-contain" src="{{ Storage::disk('s3')->url($area->icon_path) }}" alt="{{ $area->name }}">
                                            @else
