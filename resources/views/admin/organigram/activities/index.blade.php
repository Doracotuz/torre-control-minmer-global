            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border border-gray-200 p-8">
                    ...
                    <div x-data="{ editingActivity: null, newActivityName: '', newActivityDescription: '' }">
                        ...
                        <form x-bind:action="editingActivity ? '{{ route('admin.organigram.activities.update', '') }}/' + editingActivity.id : '{{ route('admin.organigram.activities.store') }}'" method="POST" class="space-y-4">
                            @csrf
                            ...
                            <div class="flex items-center justify-end">
                                <button type="button" x-show="editingActivity" @click="editingActivity = null; newActivityName = ''; newActivityDescription = '';" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 focus:bg-gray-300 active:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 mr-3">
                                    {{ __('Cancelar Edición') }}
                                </button>
                                <x-primary-button>
                                        ...
                                        @foreach ($activities as $activity)
                                            <tr class="hover:bg-gray-100 transition-colors duration-150">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                                    <button type="button" @click="editingActivity = {{ $activity }}; newActivityName = '{{ $activity->name }}'; newActivityDescription = '{{ $activity->description }}';" class="text-indigo-600 hover:text-indigo-900 transition-colors duration-200">Editar</button>
