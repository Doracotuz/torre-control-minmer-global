<x-app-layout>
    ...
    <style>
        #chart-container {
            ...
            text-align: center;
            position: relative; /* Necesario para el posicionamiento absoluto del spinner y el organigrama */
            overflow: hidden; /* CORRECCIÓN: Evita que el contenido interno redimensione el contenedor */
        }
        .orgchart {
            background: transparent !important;
            display: inline-block;
            position: absolute; /* CORRECCIÓN: Posicionamiento absoluto para controlar centrado con transform */
        }
        .orgchart .node {
    ...
    {{-- Layout y Modales --}}
    {{-- CORRECCIÓN: Se ajusta la estructura flex para que el contenedor del organigrama mantenga su tamaño --}}
    <div class="bg-gray-100 w-full h-full flex flex-col p-6">
        <div class="bg-white w-full h-full shadow-xl sm:rounded-lg border border-gray-200 flex flex-col">
            ...
            {{-- El contenedor ahora es un bloque que no se estira con el contenido --}}
            <div id="chart-container" class="w-full flex-1 relative"></div>
            ...
            <div x-data="memberModal" @open-member-modal.window="openModal($event.detail)" x-show="showModal" x-transition class="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 z-50" style="display: none;" @click.away="showModal = false">
