    <x-app-layout>
        <x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Gestión de Habilidades del Organigrama') }}
            </h2>
        </x-slot>
        ...
        <div class="py-12 bg-gray-100">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border border-gray-200 p-8">
                    ...
                    <!-- Formulario para Crear/Editar Habilidad -->
                    <div x-data="{ editingSkill: null, newSkillName: '', newSkillDescription: '' }">
                        <h3 class="text-xl font-semibold text-[#2c3856] mb-4" style="font-family: 'Raleway', sans-serif;" x-text="editingSkill ? 'Editar Habilidad' : 'Crear Nueva Habilidad'"></h3>

                        <form x-bind:action="editingSkill ? '{{ route('admin.organigram.skills.update', '') }}/' + editingSkill.id : '{{ route('admin.organigram.skills.store') }}'" method="POST" class="space-y-4">
                            @csrf
                            <template x-if="editingSkill">@method('PUT')</template>

                            <div>
                                <x-input-label for="name" :value="__('Nombre de la Habilidad')" />
                                <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" x-model="newSkillName" required />
                                <x-input-error class="mt-2" :messages="$errors->get('name')" />
                            </div>

                            <div>
                                <x-input-label for="description" :value="__('Descripción (Opcional)')" />
                                <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" x-model="newSkillDescription"></textarea>
                                <x-input-error class="mt-2" :messages="$errors->get('description')" />
                            </div>

                            <div class="flex items-center justify-end">
                                <button type="button" x-show="editingSkill" @click="editingSkill = null; newSkillName = ''; newSkillDescription = '';" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 focus:bg-gray-300 active:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150 mr-3">
                                    {{ __('Cancelar Edición') }}
                                </button>
                                <x-primary-button>
                                    <span x-text="editingSkill ? 'Actualizar Habilidad' : 'Crear Habilidad'"></span>
                                </x-primary-button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="border-t border-gray-200 pt-6 mt-6">
                        <h3 class="text-xl font-semibold text-[#2c3856] mb-4" style="font-family: 'Raleway', sans-serif;">{{ __('Lista de Habilidades') }}</h3>
                        @if ($skills->isEmpty())
                                        ...
                                        @foreach ($skills as $skill)
                                            <tr class="hover:bg-gray-100 transition-colors duration-150">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                                    <button type="button" @click="editingSkill = {{ $skill }}; newSkillName = '{{ $skill->name }}'; newSkillDescription = '{{ $skill->description }}';" class="text-indigo-600 hover:text-indigo-900 transition-colors duration-200">Editar</button>
                                                    <form action="{{ route('admin.organigram.skills.destroy', $skill) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de que quieres eliminar esta habilidad?');">
                                                        @csrf
                                                        @method('DELETE')
