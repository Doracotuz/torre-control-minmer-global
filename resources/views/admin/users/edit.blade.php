    ...
    <div class="py-6 sm:py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border border-gray-200 p-4 sm:p-8">
                <form method="POST" action="{{ route('admin.users.update', $user) }}" enctype="multipart/form-data"> {{-- ¡Importante: enctype para subir archivos! --}}
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        <label for="is_area_admin" class="inline-flex items-center">
                            <input id="is_area_admin" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="is_area_admin" value="1" {{ old('is_area_admin', $user->is_area_admin) ? 'checked' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        @if ($user->profile_photo_path)
                            <div class="mt-3 flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                                ...
                                <img src="{{ asset('storage/' . $user->profile_photo_path) }}" alt="{{ $user->name }} Profile Photo" class="h-10 w-10 object-cover rounded-full border border-gray-200 p-0.5 flex-shrink-0 object-center">
                                <label for="remove_profile_photo" class="flex items-center text-red-600 text-sm cursor-pointer hover:text-red-800">
                                    <input type="checkbox" name="remove_profile_photo" id="remove_profile_photo" value="1" class="rounded border-gray-300 text-red-600 shadow-sm focus:ring-red-500 mr-1">
                                    {{ __('Eliminar foto actual') }}
                                </label>
                            </div>
                        @else
                            <div class="mt-3 flex items-center space-x-2">
                                <p class="text-sm text-gray-600">{{ __('No hay foto de perfil actual.') }}</p>
                            </div>
                        @endif
                    </div>
