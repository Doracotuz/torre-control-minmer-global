                            ...
                            <div class="bg-white rounded-lg shadow border border-gray-100 p-4 flex flex-col text-center transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                                <div class="flex-grow flex flex-col items-center">
                                    <img class="h-20 w-20 rounded-full object-cover mb-3 border-4 
                                        ...
                                        src="{{ $user->profile_photo_path ? Storage::url($user->profile_photo_path) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&color=2C3856&background=F3F4F6' }}" 
                                        alt="{{ $user->name }}">
                                ...
                                @forelse ($users as $user)
                                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                                        <td class="px-6 py-4 whitespace-nowrap"><div class="flex items-center">
                                            {{-- CAMBIO PARA S3: Usar Storage::url para cargar la imagen desde S3 --}}
                                            <img class="h-9 w-9 rounded-full object-cover mr-3" src="{{ $user->profile_photo_path ? Storage::url($user->profile_photo_path) : 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&color=2C3856&background=F3F4F6' }}" alt="{{ $user->name }}">
                                            <div class="text-sm font-medium text-gray-900">{{ $user->name }}</div></div></td>
