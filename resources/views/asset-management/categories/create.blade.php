...
<div class="w-full max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="mb-8">
        <a href="{{ route('asset-management.categories.index') }}" class="text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-primary)] transition-colors mb-2 inline-block">
            <i class="fas fa-arrow-left mr-2"></i> Volver al listado
        </a>
        <h1 class="text-4xl font-bold text-[var(--color-text-primary)] tracking-tight">Añadir Nueva Categoría</h1>
        <p class="text-[var(--color-text-secondary)] mt-1">Crea una nueva categoría para organizar tus activos.</p>
    </div>
    ...
    <div class="bg-white p-8 rounded-xl shadow-lg mt-8">
        <form action="{{ route('asset-management.categories.store') }}" method="POST">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" class="form-input w-full" placeholder="Ej: Laptop, Monitor, Teclado" required autofocus>
            </div>
            
            <div class="mt-8 pt-6 border-t border-gray-200 flex justify-end items-center space-x-4">
                <a href="{{ route('asset-management.categories.index') }}" class="btn btn-secondary">Cancelar</a>
