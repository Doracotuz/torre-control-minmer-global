...
<div class="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
        <div>
            ...
            <h1 class="text-4xl font-bold text-[var(--color-text-primary)] tracking-tight">Gestionar Fabricantes</h1>
            <p class="text-[var(--color-text-secondary)] mt-1">Administra los fabricantes de tu inventario de hardware.</p>
        </div>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($manufacturers as $manufacturer)
                        <tr class="table-row transition-colors hover:bg-gray-50">
                            <td class="p-4 font-semibold text-[var(--color-text-primary)]">{{ $manufacturer->name }}</td>
                            <td class="p-4">
                                <div class="flex items-center justify-end space-x-4">
                                    <a href="{{ route('asset-management.manufacturers.edit', $manufacturer) }}" class="text-gray-500 hover:text-[var(--color-primary)] transition-colors" title="Editar">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <form action="{{ route('asset-management.manufacturers.destroy', $manufacturer) }}" method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este fabricante?');">
                                        @csrf
                                        @method('DELETE')
