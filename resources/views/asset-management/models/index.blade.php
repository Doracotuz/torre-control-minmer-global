...
<div class="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
        <div>
            ...
            <h1 class="text-4xl font-bold text-[var(--color-text-primary)] tracking-tight">Gestionar Modelos de Hardware</h1>
            <p class="text-[var(--color-text-secondary)] mt-1">Define los modelos específicos de hardware que utilizas.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="table-header" style="background-color: #f8f9fa;">
                    <tr>
                        <th class="p-4 font-semibold text-left text-gray-600 uppercase">Nombre del Modelo</th>
                        <th class="p-4 font-semibold text-left text-gray-600 uppercase">Fabricante</th>
                        <th class="p-4 font-semibold text-left text-gray-600 uppercase">Categoría</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($models as $model)
                        <tr class="table-row transition-colors hover:bg-gray-50">
                            <td class="p-4 font-semibold text-[var(--color-text-primary)]">{{ $model->name }}</td>
                            <td class="p-4 text-[var(--color-text-secondary)]">{{ $model->manufacturer->name }}</td>
                            <td class="p-4 text-[var(--color-text-secondary)]">{{ $model->category->name }}</td>
                            <td class="p-4">
                                <div class="flex items-center justify-end space-x-4">
                                    <a href="{{ route('asset-management.models.edit', $model) }}" class="text-gray-500 hover:text-[var(--color-primary)] transition-colors" title="Editar">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <form action="{{ route('asset-management.models.destroy', $model) }}" method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este modelo?');">
                                        @csrf
                                        @method('DELETE')
