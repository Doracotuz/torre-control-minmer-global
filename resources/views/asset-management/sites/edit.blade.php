...
<div class="w-full max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-4xl font-bold text-[var(--color-text-primary)] tracking-tight">Editar Sitio</h1>
        <p class="text-[var(--color-text-secondary)] mt-1">Modifica los detalles del sitio seleccionado.</p>
    </div>
        ...
        <form action="{{ route('asset-management.sites.update', $site) }}" method="POST">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" class="form-input w-full" value="{{ old('name', $site->name) }}" required autofocus>
                </div>
                <div>
                    <label for="address" class="form-label">Dirección (Opcional)</label>
                    <input type="text" id="address" name="address" class="form-input w-full" value="{{ old('address', $site->address) }}">
                </div>
            </div>
