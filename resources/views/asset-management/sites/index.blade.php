...
<div class="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
        <div>
            ...
            <h1 class="text-4xl font-bold text-[var(--color-text-primary)] tracking-tight">Gestionar Sitios</h1>
            <p class="text-[var(--color-text-secondary)] mt-1">Administra las ubicaciones físicas de tus activos.</p>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="table-header" style="background-color: #f8f9fa;">
                    <tr>
                        ...
                        <th class="p-4 font-semibold text-left text-gray-600 uppercase">Dirección</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($sites as $site)
                        <tr class="table-row transition-colors hover:bg-gray-50">
                            <td class="p-4 font-semibold text-[var(--color-text-primary)]">{{ $site->name }}</td>
                            <td class="p-4 text-[var(--color-text-secondary)]">{{ $site->address ?? 'N/A' }}</td>
                            <td class="p-4">
                                <div class="flex items-center justify-end space-x-4">
                                    <a href="{{ route('asset-management.sites.edit', $site) }}" class="text-gray-500 hover:text-[var(--color-primary)] transition-colors" title="Editar">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                    <form action="{{ route('asset-management.sites.destroy', $site) }}" method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este sitio?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center p-12">
                                ...
                                <p class="text-gray-500">No hay sitios registrados.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
