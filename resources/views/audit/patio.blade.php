...
<div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8" 
     x-data="{
        presentaManiobra: {{ old('presenta_maniobra', $guia->audit_patio_presenta_maniobra) ? 'true' : 'false' }},
        ...
        <form action="{{ route('audit.patio.store', $guia->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div><label class="block text-sm font-medium text-gray-700">Operador</label><input type="text" name="operador" value="{{ old('operador', $guia->operador) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required></div>
                    <div><label class="block text-sm font-medium text-gray-700">Placas</label><input type="text" name="placas" value="{{ old('placas', $guia->placas) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required></div>
                ...
                <div class="space-y-4 bg-gray-50 p-4 rounded-lg border">
                    ...
                    <div><label class="block text-sm font-medium text-gray-700">Estado de la Caja</label><select name="caja_estado" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required><option @selected(old('caja_estado') == 'Bueno')>Bueno</option><option @selected(old('caja_estado') == 'Regular')>Regular</option><option @selected(old('caja_estado') == 'Malo')>Malo</option></select></div>
                    <div><label class="block text-sm font-medium text-gray-700">Estado de las Llantas</label><select name="llantas_estado" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required><option @selected(old('llantas_estado') == 'Bueno')>Bueno</option><option @selected(old('llantas_estado') == 'Regular')>Regular</option><option @selected(old('llantas_estado') == 'Malo')>Malo</option></select></div>
                    <div><label class="block text-sm font-medium text-gray-700">Nivel de Combustible</label><select name="combustible_nivel" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required><option @selected(old('combustible_nivel') == 'Lleno')>Lleno</option><option @selected(old('combustible_nivel') == '3/4')>3/4</option><option @selected(old('combustible_nivel') == '1/2')>1/2</option><option @selected(old('combustible_nivel') == '1/4')>1/4</option><option @selected(old('combustible_nivel') == 'Reserva')>Reserva</option></select></div>
                    <div><label class="block text-sm font-medium text-gray-700">Equipo de Sujeción</label><select name="equipo_sujecion" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required><option @selected(old('equipo_sujecion') == 'No aplica')>No aplica</option><option @selected(old('equipo_sujecion') == 'Barras logísticas')>Barras logísticas</option><option @selected(old('equipo_sujecion') == 'Bandas')>Bandas</option><option @selected(old('equipo_sujecion') == 'Eslingas')>Eslingas</option><option @selected(old('equipo_sujecion') == 'Ambas')>Ambas</option></select></div>
                    
                    {{-- CAMPO CONDICIONAL PARA MANIOBRA --}}
                    <div class="pt-4 border-t">
                        ...
                        <div x-show="presentaManiobra" x-transition class="mt-4">
                            ...
                            <input type="number" name="maniobra_personas" value="{{ old('maniobra_personas', $guia->audit_patio_maniobra_personas) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" min="1">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="file" name="foto_unidad" x-ref="fotoUnidadInput" @change="previewFile($event, 'fotoUnidadPreview')" class="hidden" accept="image/*" required>
                        <div @click="$refs.fotoUnidadInput.click()" class="cursor-pointer border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-indigo-500 transition-colors">
                            <template x-if="!fotoUnidadPreview">
                                <div>
                                    ...
                                    <p class="mt-2 text-sm text-gray-600">Clic para seleccionar imagen</p>
                                </div>
                            </template>
                            <template x-if="fotoUnidadPreview">
                    ...
                    <div>
                        ...
                        <input type="file" name="foto_llantas" x-ref="fotoLlantasInput" @change="previewFile($event, 'fotoLlantasPreview')" class="hidden" accept="image/*" required>
                        <div @click="$refs.fotoLlantasInput.click()" class="cursor-pointer border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-indigo-500 transition-colors">
                            <template x-if="!fotoLlantasPreview">
                                <div>
                                    <i class="fas fa-dot-circle text-4xl text-gray-400"></i>
                                    <p class="mt-2 text-sm text-gray-600">Clic para seleccionar imagen</p>
                                </div>
                            </template>
                            <template x-if="fotoLlantasPreview">
                ...
                <div class="pt-4 border-t">
                    <button type="submit" class="w-full px-6 py-3 bg-purple-600 text-white rounded-lg font-bold shadow-lg hover:bg-purple-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
                        Confirmar Arribo y Poner en Cortina
                    </button>
                </div>
            </div>
        </form>
