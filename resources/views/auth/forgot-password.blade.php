<x-guest-layout>
    ...
    <div class="mb-4 text-sm text-gray-600 text-center"> {{-- Añadido text-center para mejor alineación --}}
        {{ __('Para restablecer tu contraseña, proporciona tu correo electrónico.') }}
    </div>

    <x-auth-session-status class="mb-4 text-center text-sm text-red-600" :status="session('status')" /> {{-- Alineado el estilo con login.blade.php --}}
    ...
    <form method="POST" action="{{ route('password.email') }}">
        ...
        <div class="mb-5"> {{-- Ajuste de espaciado para coincidir con login.blade.php --}}
            ...
            <x-text-input id="email" class="block mt-2 w-full px-4 py-2 rounded-lg border-gray-300 focus:border-[#ff9c00] focus:ring-[#ff9c00] transition-all duration-200 shadow-sm" type="email" name="email" :value="old('email')" required autofocus /> {{-- Alineado el estilo con login.blade.php --}}
