                ...
                <tr>
                    ...
                    <template x-for="(columnKey, index) in columnOrder" :key="`${columnKey}-${index}`">
                        <th x-show="visibleColumns[columnKey]"
                            ...
                            class="px-2 py-1 text-center text-xs font-medium text-white uppercase tracking-wider select-none border-l border-gray-200 text-wrap">
                            <span class="drag-handle cursor-move" x-text="allColumns[columnKey]"></span>
                            <div class="resizer no-drag"></div>
                        </th>
                    </template>
                    <th class="px-2 py-1 text-center text-xs font-medium text-white uppercase tracking-wider no-drag border-l border-gray-200">
                        Acciones
                    </th>
                </tr>
            </thead>
                ...
                <template x-for="order in orders" :key="order.id">
                    <tr class="hover:bg-gray-100" :class="{'bg-blue-50': selectedOrders.includes(order.id)}">
                        <td class="px-2 py-1 border text-center border-gray-200">
                            <input type="checkbox" :value="order.id" x-model="selectedOrders" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        </td>

                        <template x-for="columnKey in columnOrder" :key="columnKey">
                            <td x-show="visibleColumns[columnKey]" 
                                class="px-2 py-1 whitespace-nowrap text-sm text-gray-500 border border-gray-200">
                                ...
                                <template x-if="columnKey !== 'status'">
                                    <span :title="getFormattedCell(order, columnKey)" 
