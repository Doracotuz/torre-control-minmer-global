            ...
            <div class="lg:col-span-2 bg-white overflow-hidden shadow-xl sm:rounded-lg p-8">
                <div class="flex justify-between items-start mb-6">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">SO: {{ $planning->so_number }} | Factura: {{ $planning->factura ?? 'N/A' }}</p>
                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-6 text-sm border-b pb-6">
                    <div><strong class="block text-gray-500">Origen:</strong><span>{{ $planning->origen }}</span></div>
                    <div><strong class="block text-gray-500">Destino:</strong><span>{{ $planning->destino }}</span></div>
                    <div><strong class="block text-gray-500">Fecha Entrega:</strong><span>{{ $planning->fecha_entrega?->format('d/m/Y') ?? 'N/A' }}</span></div>
                    <div class="col-span-full"><strong class="block text-gray-500">Dirección:</strong><span>{{ $planning->direccion }}</span></div>
                ...
                <div class="max-h-48 overflow-y-auto border rounded-lg">
                 ...
                 <div class="mt-8 flex justify-end gap-4">
                    <a href="{{ route('customer-service.planning.edit', $planning) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-semibold shadow-sm hover:bg-indigo-700">Editar Planificación</a>
                </div>
            </div>
