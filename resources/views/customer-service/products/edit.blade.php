<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editando Producto: <span class="text-blue-600">{{ $product->sku }}</span></h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-8">
                <form action="{{ route('customer-service.products.update', $product) }}" method="POST" x-data="{ sku: '{{ old('sku', $product->sku) }}' }">
                    @csrf
                    @method('PUT')
                    @if ($errors->any())
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="3" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('description', $product->description) }}</textarea>
                        </div>
                        <div>
                            <label for="packaging_factor" class="block text-sm font-medium text-gray-700">F. Empaque</label>
                            <input type="number" name="packaging_factor" id="packaging_factor" value="{{ old('packaging_factor', $product->packaging_factor) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                        </div>
                        <div>
                            ...
                            <select id="select-brand" name="cs_brand_id" required placeholder="Busca o crea una marca...">
                                ...
                                @foreach($brands as $brand)
                                    <option value="{{ $brand->id }}" {{ old('cs_brand_id', $product->cs_brand_id) == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="flex justify-end gap-4 mt-8">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Actualizar Producto</button>
                    </div>
                </form>
            </div>
        </div>
