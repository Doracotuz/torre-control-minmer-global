                        ...
                        <nav class="flex-1 px-4 py-6 space-y-2">
                            ...
                            @if(Auth::user()->hasModuleAccess('routes'))
                                <x-nav-link :href="route('rutas.dashboard')" :active="request()->routeIs('rutas.*')" class="nav-link-custom {{ request()->routeIs('rutas.*') ? 'active-link' : '' }}">
                                    <svg class="nav-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 6.75V15m0 0v2.25m0-2.25h1.5m-1.5 0H5.25m11.25-8.25v2.25m0-2.25h-1.5m1.5 0H12m0 0v2.25m0-2.25V6.75m0 0H9m12 6.75h-1.5m1.5 0v-2.25m0 2.25H12m0 0H9m-3.75 0H5.25m0 0V9.75M5.25 12h1.5m0 0V9.75m0 0H5.25m3.75 0H9m-3.75 0H5.25m0 0h1.5m3 0h1.5m-1.5 0H9m-3.75 0H9m9 3.75h1.5m-1.5 0H9m3.75 0H9m-3.75 0H9" />
                                    </svg>
                            ...
                            @if(Auth::user()->hasModuleAccess('electronic_label'))
                                <x-nav-link :href="route('electronic-label.index')" :active="request()->routeIs('electronic-label.*')" class="nav-link-custom {{ request()->routeIs('electronic-label.*') ? 'active-link' : '' }}">
                                    <svg class="nav-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6z" />
                                    </svg>
                                    <span class="nav-text">{{ __('Marbete Electrónico') }}</span>
                                </x-nav-link>
                            @endif                            
                                ...
                                <div class="pt-4 mt-2 border-t border-white/10">
                                    ...
                                    <div x-show="isSuperAdminMenuOpen" x-transition:enter="transition ease-out duration-200" ...>
                                        <div class="pl-4 mt-2 space-y-2">
                                            ...
                                            <x-nav-link :href="route('admin.roles.index')" :active="request()->routeIs('admin.roles.*')" class="nav-link-custom {{ request()->routeIs('admin.roles.*') ? 'active-link' : '' }}">
                                                <svg class="nav-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            ...
                                            @php
                                                $unreadNotificationsCount = \App\Models\SyncNotification::where('resolved', false)->count();
                                            @endphp
                                            <x-nav-link :href="route('admin.sync-notifications.index')" :active="request()->routeIs('admin.sync-notifications.*')" class="nav-link-custom {{ request()->routeIs('admin.sync-notifications.*') ? 'active-link' : '' }} flex justify-between items-center group">
                                                <div class="flex items-center">
                                                    <svg class="nav-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0M3.124 7.5A8.969 8.969 0 015.292 3m13.416 0a8.969 8.969 0 012.168 4.5" />
                                                    </svg>
                                                    <span class="nav-text">{{ __('Sync Notifications') }}</span>
                                                </div>
                                                @if($unreadNotificationsCount > 0)
                                                    <span class="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-sm group-hover:scale-110 transition-transform">
                                    ...
                                    <div x-show="isAreaAdminMenuOpen"
                                        ...
                                        <div class="pl-4 mt-2 space-y-2">
                                            ...
                                            <x-nav-link :href="route('area_admin.users.index')" :active="request()->routeIs('area_admin.users.*')" class="nav-link-custom {{ request()->routeIs('area_admin.users.*') ? 'active-link' : '' }}">
                                                <svg class="nav-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a8.967 8.967 0 0015 0H4.501z" /></svg>
