                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                             ...
                             <select name="leader_id" id="leader_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('leader_id', $project->leader_id) == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                             </select>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                @foreach (['Planeación', 'En Progreso', 'En Pausa', 'Completado', 'Cancelado'] as $status)
                        ...
                        <div>
                            <label for="budget" class="block text-sm font-medium text-gray-700">Presupuesto del Proyecto ($)</label>
