<x-app-layout>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('monitoringManager', () => ({
            ...
            // --- INICIA CORRECCIÓN: Objeto de subtipos completo ---
            eventSubtypes: {
            ...
            closeAllModals() { this.isDetailsModalOpen = false; this.isEventModalOpen = false; this.isReportModalOpen = false; this.isColumnSelectorOpen = false; },
            ...
            startRouteFromMonitor() { if (!this.selectedGuia) return; if (!confirm(`Iniciar ruta para guía ${this.selectedGuia.guia}?`)) return; this.isSubmitting = true; fetch(`/rutas/monitoreo/${this.selectedGuia.id}/start`, { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'), 'Accept': 'application/json' } }).then(r => r.json()).then(d => { if (d.success) { this.showNotification(d.message, 'success'); this.closeAllModals(); this.applyFilters(); } else { this.showNotification(d.message || 'Error.', 'error'); } }).catch(() => this.showNotification('Error de conexión.', 'error')).finally(() => this.isSubmitting = false); },
            submitEventForm() {
