<x-app-layout>
    ...
    <div class="relative w-full h-[calc(100vh-64px)] bg-[#2c3856] overflow-hidden font-['Montserrat',sans-serif]">
        ...
        <div class="absolute top-4 left-4 z-20 flex flex-col space-y-3">
            <div class="bg-white/90 backdrop-blur-xl border border-white/40 p-2 rounded-xl shadow-lg flex flex-col space-y-2">
                
                <button type="button" onclick="window.toggleMapStyle()" class="flex items-center space-x-2 px-3 py-2 rounded-lg hover:bg-orange-50 transition-colors duration-300 group">
                ...
                <button type="button" onclick="window.toggleTraffic()" class="flex items-center space-x-2 px-3 py-2 rounded-lg hover:bg-orange-50 transition-colors duration-300 group">
                ...
                <button type="button" onclick="window.toggleSatellite()" class="flex items-center space-x-2 px-3 py-2 rounded-lg hover:bg-orange-50 transition-colors duration-300 group">
                    <div class="p-1.5 rounded-md bg-gray-200 text-gray-500 group-hover:text-white group-hover:bg-[#ff9c00] transition-colors" id="icon-satellite-bg">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                    </div>
                    <span class="text-sm font-bold text-[#2c3856] hidden sm:inline"><span id="satellite-label">Satélite</span></span>
                </button>
            </div>
        </div>
        ...
        <div class="absolute bottom-0 left-0 w-full h-[50vh] md:top-0 md:right-0 md:w-[480px] md:h-full md:left-auto bg-white/80 backdrop-blur-xl border-t md:border-t-0 md:border-l border-white/30 shadow-[-10px_0_30px_rgba(0,0,0,0.1)] overflow-y-auto transition-all duration-500 ease-out p-6 md:p-8 rounded-t-[2rem] md:rounded-none z-30">
            ...
            <form action="{{ route('rutas.plantillas.update', $ruta) }}" method="POST" id="rutaForm" class="space-y-8 pb-20 md:pb-0">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4 animate-fade-in-down">
                    ...
                    <h2 class="font-['Raleway',sans-serif] font-extrabold text-2xl md:text-3xl text-[#2c3856] leading-tight">
                        Editar: <span class="text-[#ff9c00]">{{ $ruta->nombre }}</span>
                    </h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="relative">
                            <select name="region" id="region" required
                                ...
                                @foreach(['MEX', 'SJD', 'GDL', 'MTY', 'CUN', 'MIN', 'MZN', 'VER'] as $regionOption)
                                    <option value="{{ $regionOption }}" {{ old('region', $ruta->region) == $regionOption ? 'selected' : '' }}>{{ $regionOption }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="relative">
                            <select name="tipo_ruta" id="tipo_ruta" required
                                    class="block px-4 py-3 w-full text-[#2b2b2b] bg-white/50 backdrop-blur-sm rounded-lg border-2 border-gray-200/50 appearance-none focus:outline-none focus:ring-0 focus:border-[#ff9c00] focus:bg-white/80 transition-all duration-300">
                                <option value="Entrega" {{ old('tipo_ruta', $ruta->tipo_ruta) == 'Entrega' ? 'selected' : '' }}>Entrega</option>
                                <option value="Traslado" {{ old('tipo_ruta', $ruta->tipo_ruta) == 'Traslado' ? 'selected' : '' }}>Traslado</option>
                                <option value="Importacion" {{ old('tipo_ruta', $ruta->tipo_ruta) == 'Importacion' ? 'selected' : '' }}>Importación</option>
                            </select>
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-[#666666]">
                                <svg class="fill-current h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                            </div>
                ...
                <div class="space-y-6 animate-fade-in-up delay-300">
                    <div>
                        ...
                        <div class="relative group z-50">
                            ...
                            <input type="text" id="autocomplete" placeholder="Buscar lugar para añadir..." 
                    ...
                    <div id="resumen-ruta" class="mt-6 p-5 bg-gradient-to-br from-[#2c3856]/10 to-white/50 rounded-xl border border-white/40 shadow-sm animate-fade-in">
                        <h4 class="font-['Raleway',sans-serif] font-bold text-[#2c3856] mb-1">Resumen</h4>
                        <div class="flex items-baseline">
                            <span class="text-sm text-[#666666] mr-2">Distancia Total:</span>
                            <span id="distancia-total" class="text-2xl font-extrabold text-[#ff9c00] tracking-tight">{{ $ruta->distancia_total_km }} km</span>
                        </div>
                    </div>
                    ...
                    <div class="pt-4">
                        <button type="button" onclick="window.validarYEnviarFormulario()" 
                            ...
                            <span class="relative">Actualizar Ruta</span>
                        </button>
                    </div>
                </div>
            </form>
    ...
    <style>
        ...
        .sortable-chosen { box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.1); }
    </style>
    ...
    <script>
        (function() {
            var map, directionsService, directionsRenderer, trafficLayer;
            var markers = [];
            var paradas = [];
            var mapType = 'roadmap';
            ...
            var initialParadas = @json($initialParadas);
            ...
            window.toggleMapStyle = function() {
                ...
                updateMapOptions();
                document.getElementById('style-label').innerText = isMinmerStyle ? "Minmer" : "Natural";
            };
            ...
            window.toggleTraffic = function() {
                ...
                if (isTrafficOn) {
                    if(trafficLayer && map) trafficLayer.setMap(map);
                    bgIcon.classList.replace('bg-gray-200', 'bg-[#ff9c00]');
                    bgIcon.classList.replace('text-gray-500', 'text-white');
                } else {
                    if(trafficLayer) trafficLayer.setMap(null);
                    bgIcon.classList.replace('bg-[#ff9c00]', 'bg-gray-200');
                    bgIcon.classList.replace('text-white', 'text-gray-500');
                }
            };
            ...
            window.toggleSatellite = function() {
                ...
                updateMapOptions();
                const bgIcon = document.getElementById('icon-satellite-bg');
                const label = document.getElementById('satellite-label');
                
                if (mapType === 'hybrid') {
                    bgIcon.classList.replace('bg-gray-200', 'bg-[#ff9c00]');
                    bgIcon.classList.replace('text-gray-500', 'text-white');
                    label.innerText = "Mapa";
                } else {
                    bgIcon.classList.replace('bg-[#ff9c00]', 'bg-gray-200');
                    bgIcon.classList.replace('text-white', 'text-gray-500');
                    label.innerText = "Satélite";
                }
            };
            ...
            function updateMapOptions() {
                ...
                map.setOptions({
                    mapTypeId: mapType,
                    styles: (mapType === 'roadmap' && isMinmerStyle) ? minmerStyles : null
                });
            }
            ...
            window.initMap = function() {
                ...
                if (initialParadas && Array.isArray(initialParadas)) {
                    initialParadas.forEach(p => {
                        const lat = parseFloat(p.latitud || p.lat);
                        const lng = parseFloat(p.longitud || p.lng);
                        
                        if (isValidCoordinate(lat, lng)) {
                            const loc = new google.maps.LatLng(lat, lng);
                            agregarParada(loc, p.direccion || '', p.nombre || '');
                        } else {
                            console.warn('Coordenada inválida ignorada:', p);
                        }
                    });
                }
            };
            ...
            function setupAutocomplete() {
                ...
                autocomplete.addListener('place_changed', () => {
                    ...
                    const lugarNombre = place.name || place.formatted_address;
                    agregarParada(place.geometry.location, place.formatted_address || place.name, lugarNombre);
                    input.value = ''; 
            ...
            function geocodePosition(latLng) {
                ...
                geocoder.geocode({ location: latLng }, (results, status) => {
                    if (status === "OK" && results[0]) {
                        agregarParada(latLng, results[0].formatted_address, results[0].formatted_address);
                    } else {
                        const coords = `Coordenadas: ${latLng.toUrlValue(4)}`;
                        agregarParada(latLng, coords, coords);
                    }
                });
            }

            function agregarParada(location, address, nombrePersonalizado) {
                ...
                const latVal = typeof location.lat === 'function' ? location.lat() : location.lat;
                const lngVal = typeof location.lng === 'function' ? location.lng() : location.lng;

                const paradaData = { lat: latVal, lng: lngVal, address: address, nombre: nombreFinal };
                ...
                paradaElement.innerHTML = `
                    ...
                    <button type="button" onclick="window.eliminarParada(${index})" class="mt-1 p-2 ml-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-all duration-300 opacity-60 group-hover:opacity-100">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                `;
                container.appendChild(paradaElement);
                actualizarRuta();
            }

            window.actualizarNombreParada = function(index, nuevoNombre) {
                if (paradas[index]) paradas[index].nombre = nuevoNombre;
            };

            window.eliminarParada = function(index) {
                ...
                Array.from(container.children).forEach((child, newIndex) => {
                    ...
                    child.querySelector('button').setAttribute('onclick', `window.eliminarParada(${newIndex})`);
            ...
            function actualizarRuta() {
                ...
                directionsService.route({
                    ...
                    if (status === "OK" && response.routes.length > 0) {
                        ...
                        let totalDistance = 0;
                        response.routes[0].legs.forEach(leg => { totalDistance += leg.distance.value; });
            ...
            window.validarYEnviarFormulario = function() {
                if (paradas.length < 2) {
                    const errorEl = document.getElementById('paradas-error');
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key={{ $googleMapsApiKey }}&libraries=places,drawing&loading=async&callback=initMap" async defer></script>
