    ...
    <div class="min-h-screen bg-transparent text-[#2b2b2b] font-montserrat pb-20 relative">
        ...
        <div class="max-w-4xl mx-auto px-6 pt-10 relative z-10">
            <div class="flex items-center gap-4 mb-8">
                ...
                <div>
                    <p class="text-xs font-bold text-[#666666] uppercase tracking-[0.2em] mb-1">Mapa del Almacén</p>
                    <h1 class="text-4xl font-raleway font-black text-[#2c3856]">Nueva Ubicación</h1>
                </div>
            </div>

            <div class="bg-white rounded-[2.5rem] shadow-soft border border-gray-100 p-8 md:p-12">
                <form action="{{ route('wms.locations.store') }}" method="POST">
                    @csrf
                    @include('wms.locations._form')
                    <div class="mt-12 flex justify-end items-center gap-4">
                        <a href="{{ route('wms.locations.index') }}" class="px-8 py-3 rounded-xl border border-gray-200 text-[#666666] font-bold hover:bg-gray-50 transition-all">Cancelar</a>
                        <button type="submit" class="px-10 py-3 rounded-xl bg-[#2c3856] text-white font-bold shadow-lg shadow-[#2c3856]/30 hover:bg-[#1a253a] hover:-translate-y-0.5 transition-all">Guardar Ubicación</button>
                    </div>
                </form>
            </div>
        </div>
