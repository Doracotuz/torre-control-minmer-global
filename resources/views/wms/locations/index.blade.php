<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center space-x-2">
                {{-- Formulario para imprimir etiquetas, usa el almacén global '$store.selection' --}}
                <form action="{{ route('wms.locations.print-labels') }}" method="POST" target="_blank" class="inline-block">
                    ...
                    <button type="submit" :disabled="$store.selection.ids.length === 0" class="px-4 py-2 bg-gray-700 text-white rounded-md text-sm font-semibold shadow-sm hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="p-4">
                                    <input type="checkbox" 
                                           @click="$store.selection.toggleAll({{ $locations->pluck('id')->toJson() }})"
                                           :checked="$store.selection.areAllSelected({{ $locations->pluck('id')->toJson() }})"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($locations as $location)
                                <tr class="hover:bg-gray-50">
                                    {{-- Checkbox para cada fila individual --}}
                                    <td class="p-4">
                                        <input type="checkbox" value="{{ $location->id }}" x-model="$store.selection.ids" class="rounded border-gray-300">
                                    </td>
    ...
    {{-- Script para inicializar el Almacén Global de Alpine.js --}}
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.store('selection', {
                // El array donde se guardan los IDs
                ids: [],

                // Lógica para el botón "seleccionar todo"
                toggleAll(allIds) {
                    // Si ya están todos seleccionados, vacía el array. Si no, lo llena.
                    if (this.ids.length === allIds.length) {
                        this.ids = [];
                    } else {
                        // Se usa Set para evitar duplicados y se convierte de nuevo a array
                        this.ids = [...new Set(allIds)];
                    }
                },

                // Lógica para saber si el checkbox "seleccionar todo" debe estar marcado
                areAllSelected(allIds) {
                    if (!allIds || allIds.length === 0) return false;
                    return this.ids.length === allIds.length;
                }
            })
        })
    </script>
