        ...
        <div class="max-w-4xl mx-auto px-6 pt-10 relative z-10">
            <div class="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <h1 class="text-4xl font-raleway font-black text-[#2c3856]">Editar Tipo</h1>
                </div>
            </div>

            <div class="bg-white rounded-[2.5rem] shadow-soft border border-gray-100 p-8 md:p-12">
                <form action="{{ route('wms.product-types.update', $productType) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="relative">
                            <select name="area_id" id="area_id" required class="block w-full pl-4 pr-4 py-4 rounded-xl border-[#2c3856]/20 bg-white text-[#2c3856] font-bold text-lg focus:border-[#2c3856] focus:ring-[#2c3856] transition-all cursor-pointer shadow-sm">
                                ...
                                @foreach($areas as $area)
                                    <option value="{{ $area->id }}" {{ (old('area_id', $productType->area_id) == $area->id) ? 'selected' : '' }}>{{ $area->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="mb-8">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $productType->name) }}" required class="w-full px-4 py-3 rounded-xl border-gray-200 bg-gray-50 focus:bg-white focus:border-[#2c3856] focus:ring-[#2c3856] transition-all text-lg font-medium text-[#2c3856] shadow-inner">
                    ...
                    <div class="mt-12 flex justify-end items-center gap-4">
                        ...
                        <button type="submit" class="px-10 py-3 rounded-xl bg-[#2c3856] text-white font-bold shadow-lg shadow-[#2c3856]/30 hover:bg-[#1a253a] hover:-translate-y-0.5 transition-all">Actualizar Tipo</button>
                    </div>
                </form>
            </div>
        </div>
