        ...
        <div class="max-w-[1800px] mx-auto px-6 pt-10 relative z-10">
            ...
            <div class="flex flex-col xl:flex-row justify-between items-end mb-10 border-b border-gray-200 pb-6">
                <div>
                    ...
                    <h1 class="text-5xl font-raleway font-black text-[#2c3856] mb-1 leading-none">
                        Tipos de <span class="text-[#ff9c00]">Producto</span>
                    </h1>
                </div>
                ...
                <div class="flex flex-wrap gap-3 mt-6 xl:mt-0">
                    ...
                    <a href="{{ route('wms.product-types.create') }}" class="flex items-center gap-2 px-6 py-2.5 bg-[#2c3856] text-white font-bold rounded-full shadow-lg shadow-[#2c3856]/20 hover:bg-[#1a253a] hover:-translate-y-0.5 transition-all">
                        <i class="fas fa-plus"></i> <span>Nuevo Tipo</span>
                    </a>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2rem] shadow-soft border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100 bg-gray-50/50">
                    <form method="GET" action="{{ route('wms.product-types.index') }}" class="flex flex-col md:flex-row gap-4 items-end">
                        ...
                        <div class="w-full md:w-1/3">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="search" placeholder="Nombre del tipo..." value="{{ request('search') }}" class="w-full pl-10 pr-4 py-3 rounded-xl border-gray-200 bg-white focus:border-[#ff9c00] focus:ring-[#ff9c00] transition-all">
                            </div>
                        </div>
                        <div class="flex gap-2">
                            ...
                            <a href="{{ route('wms.product-types.index') }}" class="w-12 flex items-center justify-center py-3 bg-white border border-gray-200 text-[#666666] rounded-xl hover:bg-gray-50 transition-all" title="Limpiar">
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-bold text-[#666666] uppercase tracking-wider">Nombre del Tipo</th>
                                ...
                                <tr class="hover:bg-blue-50/30 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-[#fff8e6] text-[#b36b00] border border-[#ff9c00]/20">
                                            {{ $productType->area->name ?? 'Global' }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap font-medium text-[#2c3856]">{{ $productType->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('wms.product-types.edit', $productType) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-white border border-gray-200 text-gray-400 hover:text-[#ff9c00] hover:border-[#ff9c00] transition-all shadow-sm mr-2">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <form action="{{ route('wms.product-types.destroy', $productType) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este tipo?');">
                                            @csrf @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="3" class="px-6 py-12 text-center text-gray-400">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-sm font-medium">No hay tipos de producto registrados.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
