<x-app-layout>
    ...
    <div class="min-h-screen bg-transparent text-[#2b2b2b] font-montserrat pb-20 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 pt-10 relative z-10" x-data="salesOrderForm()" x-init="initData()" x-cloak>
            ...
            <div class="flex flex-col md:flex-row justify-between items-end mb-10 stagger-enter" style="animation-delay: 0.1s;">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        ...
                        <span class="text-sm font-bold text-[#2c3856] tracking-[0.3em] uppercase">Logística de Salida</span>
                    </div>
                    <h1 class="text-4xl md:text-5xl font-raleway font-black text-[#2c3856] leading-none">
                        Nueva Orden de <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#ff9c00] to-orange-600">Venta</span>
                    </h1>
                </div>
                <div class="mt-4 md:mt-0 flex gap-3">
                    <button @click="$refs.importInput.click()" class="btn-ghost px-6 py-3 text-sm uppercase tracking-wider">
                        <i class="fas fa-file-upload mr-2"></i> Cargar CSV
                    </button>
                    <a href="{{ route('wms.sales-orders.index') }}" class="btn-ghost px-6 py-3 text-sm uppercase tracking-wider border-red-200 text-red-600 hover:bg-red-50">
                        Cancelar
                    </a>
                </div>
            </div>

            <form action="{{ route('wms.sales-orders.import-new') }}" method="POST" enctype="multipart/form-data" class="hidden">
                @csrf
            ...
            <form method="POST" action="{{ route('wms.sales-orders.store') }}" id="create-so-form" class="stagger-enter" style="animation-delay: 0.2s;">
                @csrf
                ...
                <div class="bg-white rounded-[2.5rem] shadow-xl shadow-[#2c3856]/5 border border-gray-100 p-10 mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                        <div>
                            ...
                            <input type="text" name="so_number" value="{{ old('so_number') }}" required class="input-arch text-lg" placeholder="SO-2026-001">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Factura (Opcional)</label>
                            <input type="text" name="invoice_number" value="{{ old('invoice_number') }}" class="input-arch text-lg" placeholder="F-0000">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Fecha de Entrega</label>
                            <input type="date" name="delivery_date" value="{{ old('delivery_date', now()->format('Y-m-d')) }}" required class="input-arch text-lg">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            ...
                            <input type="text" name="customer_name" value="{{ old('customer_name') }}" required class="input-arch text-lg" placeholder="Nombre del cliente">
                        </div>
                        <div>
                ...
                <div x-data="{ tab: 'manual' }" class="bg-white rounded-[2.5rem] shadow-xl shadow-[#2c3856]/5 border border-gray-100 p-10">
                    ...
                    <div x-show="tab === 'plantilla'" class="text-center py-10">
                        <div class="max-w-lg mx-auto bg-gray-50 rounded-[2rem] p-10 border border-dashed border-gray-300">
                            ...
                            <p class="text-gray-600 mb-6 font-medium">Carga masiva de líneas mediante CSV. Reemplazará la entrada manual.</p>
                            
                            <form action="{{ route('wms.sales-orders.import-new') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                                @csrf
                    ...
                    <div class="mt-10 pt-6 border-t border-gray-100 flex justify-end">
                        <button type="submit" 
                            ...
                            <i class="fas fa-paper-plane mr-2"></i> Crear Orden
                        </button>
                    </div>
                </div>
            </form>
                ...
                initData() {
                    ...
                    if (data.importedLines && data.importedLines.length > 0) {
                        this.lines = data.importedLines.map(line => ({
                            ...
                            searchTerm: `SKU ID: ${line.product_id}`,
                            selectedProductName: 'Producto Importado',
                            searchResults: [],
                            searchLoading: false,
                            highlightedIndex: -1,
